...
class SatelliteImageProcessor:
    ...
    def plot_original_image(self):
        if self.image.shape[0] == 1:
            img_to_plot = np.repeat(self.image[0, :, :], 3).reshape((self.image.shape[1], self.image.shape[2], 3))
...
if __name__ == "__main__":
    processor = SatelliteImageProcessor(file_path="test.tiff", output_folder="output", water_level=0)
    processor.process()
